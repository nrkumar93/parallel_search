...
def LinearPlan(robot_state, pose1, pose2, ik_solver, step_size=0.01,
    ...
    steps = int(np.ceil(max(np.linalg.norm(p2 - p1) / step_size,
                        abs(theta) / rotation_step_size)))
     
    # Handle case where 0 steps (i.e. we're at the goal already)
    if steps <= 1:
        #config = ik_solver(robot_state.ref, pq, q0=config, pose_as_matrix=False)
...
def JointSpaceLinearPlan(robot_state, q1, q2, step_size=0.05,
                         acceleration_fraction=1., *args, **kwargs):
