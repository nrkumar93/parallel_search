...
drawer_to_approach_handle = {
    "indigo_drawer": [
        make_pose(([0.40, 0.0, 0.0], to_handle_q1)),
    ]
}
drawer_to_cage_handle = {
    "indigo_drawer": [
        make_pose(([0.28, 0.0, 0.0], to_handle_q1)),
    ]
}
drawer_to_release_handle = {
    "indigo_drawer": [
        make_pose(([0.38, 0.0, 0.08], to_handle_q1)),
        # make_pose(([0.38, 0.0, 0.08], to_handle_q2)),
    ]
}
drawer_to_push_handle = {
    "indigo_drawer": [
        [make_pose(([0.247, 0.0, 0.0], to_handle_q1))],
    ]
}
...
cabinet_to_open = {
    "indigo_drawer_top": make_pose(([0.578, -0.033, -0.079],
                                        [0.519, -0.494, -0.445, 0.536])),
    "indigo_drawer_bottom": make_pose(([0.578, -0.033, -0.210],
                                           [0.519, -0.494, -0.445, 0.536])),
}
...
backoff_step = make_pose((0, 0, -0.1), (0, 0, 0, 1))

# Potted Meat
spam_to_approach_negx = ([0.03, 0.0, 0.12], [0.033, 0.997, -0.016, -0.065])
spam_to_cage_negx = ([0.0, 0.0, 0.015], [0.033, 0.997, -0.015, -0.065])
spam_approach_pose_negx = make_pose(*spam_to_approach_negx)
spam_cage_pose_negx = make_pose(*spam_to_cage_negx)
...
ycb_approach = {
    "potted_meat_can": [
        make_pose(spam_approach_front_pos, spam_q3),
        make_pose(spam_approach_front_pos, spam_q2),
        make_pose(spam_approach_back_pos, spam_q3),
        make_pose(spam_approach_back_pos, spam_q2),
    ],
    "sugar_box": [
        # make_pose((sugar_approach_front_pos, sugar_q1)),
        make_pose(sugar_approach_top_pos, sugar_approach_top_q),
        # TODO enable this
        # make_pose((sugar_approach_front_pos, sugar_q2)),
    ],
    "tomato_soup_can": make_pose(soup_approach_pos, soup_q1),
}
ycb_cage = {
    "potted_meat_can": [
        make_pose(spam_grasp_pos, [0.035, 0.999, 0.022, -0.022]),
        make_pose([0.0, 0.0, 0.015], [0.994, 0.048, -0.095, -0.014]),
    ],
    "sugar_box": [
        make_pose((sugar_cage_top_pos, sugar_cage_top_q)),
        # make_pose(([0.0, -0.03, 0.064], sugar_q1)),
        # make_pose(([0.0, -0.03, 0.064], sugar_q2)),
        # make_pose(([0.0, 0.03, 0.064], sugar_q1)),
        # make_pose(([0.0, 0.03, 0.064], sugar_q2)),
    ],
    "tomato_soup_can": [make_pose(soup_cage_pos, soup_q1)],
}

supported_ycb_objects = ["potted_meat_can", "sugar_box", "tomato_soup_can",
                         "mustard_bottle", "cracker_box"]

ycb_over_drawer = {
    "potted_meat_can": [
        make_pose(spam_over_pos, [0.994, 0.048, -0.095, -0.014]),
        make_pose(spam_over_pos, [0.979, 0.192, 0.048, -0.051]),
    ],
    "sugar_box": [
        # make_pose((sugar_over_pos, sugar_place_rot1)),
        # make_pose((sugar_over_pos, sugar_place_rot2)),
        make_pose(sugar_over_top_pos, sugar_over_top_q),
    ],
    "tomato_soup_can": [
        make_pose(soup_over_pos, soup_place_rot)
    ],
}

ycb_place = {
    "potted_meat_can": [
        make_pose(spam_place_pos, spam_q1),
        make_pose(spam_place_pos, spam_q2),
    ],
    "sugar_box": [
        # make_pose((sugar_place_pos, sugar_place_rot1)),
        # make_pose((sugar_place_pos, sugar_place_rot2)),
        make_pose(sugar_place_top_pos, sugar_place_top_q),
    ],
    "tomato_soup_can": [
        make_pose(soup_place_pos, soup_place_rot)
    ],
}
